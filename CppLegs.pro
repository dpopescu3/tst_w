...
win32 {
    PS        = powershell -ExecutionPolicy Bypass -NoProfile -File
    PS_SCRIPT = $$shell_quote($$PWD/auto_commit.ps1)
    # RUN AFTER LINKING so the exe isn't locked
    QMAKE_POST_LINK += $$PS $$PS_SCRIPT \
        $$shell_quote($$PWD) \
        $$shell_quote($$TARGET) \
        $$shell_quote($$DESTDIR) \
        || ver>nul
